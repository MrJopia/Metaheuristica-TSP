...
# Función objetivo para Guided Local Search con selección aleatoria
def local_search_random_selection(weight: np.ndarray, init_s: list, penalty: np.ndarray, eta: float, max_unimproved: int):
    ...
    while n_unimproved < max_unimproved:
        ...
        # Generar índices aleatorios para intentar mejoras
        for _ in range(len(s)):  # Intentar tantas veces como la longitud de la solución
            a = random.randint(0, len(s) - 2)  # Selecciona un índice aleatorio para `a`
            c = random.randint(a + 1, len(s) - 2)  # Selecciona un índice aleatorio para `c`
            ...
            if after + eta * dpen < before:
                ...
                break  # Rompe el bucle interno para la primera mejora aleatoria
...
def guided_local_search(weight: np.ndarray, eta: float, max_iter=10000, max_unimproved=100):
    ...
    new_s = local_search_random_selection(weight, init_s=current_s, penalty=penalty, eta=eta, max_unimproved=max_unimproved)
