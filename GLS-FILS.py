...
# Función objetivo para Guided Local Search con la primera mejora
def local_search_first_improvement(weight: np.ndarray, init_s: list, penalty: np.ndarray, eta: float, max_unimproved: int):
    n_cities = weight.shape[0]
    s = init_s
    n_unimproved = 0

    # Añade la ciudad de inicio (ciudad 0) al final de la ruta para cerrarla
    s.append(0)

    while n_unimproved < max_unimproved:
        improved = False

        # Busca el primer intercambio que mejore la solución
        for a in range(len(s) - 1):
            for c in range(a + 1, len(s) - 1):
                # Calcula la longitud antes y después del intercambio
                before = weight[s[a], s[a+1]] + weight[s[c], s[c+1]]
                after = weight[s[a], s[c]] + weight[s[a+1], s[c+1]]
                dpen = penalty[s[a+1] - 1] - penalty[s[c] - 1]

                # Si la nueva solución es mejor, se acepta el intercambio
                if after + eta * dpen < before:
                    s[a+1], s[c] = s[c], s[a+1]  # Realiza el intercambio
                    improved = True
                    n_unimproved = 0  # Resetea el contador de no-mejoras
                    break  # Rompe el bucle interno para la primera mejora

            if improved:
                break  # Rompe el bucle externo también si hay mejora

        if not improved:
            n_unimproved += 1

    # Remueve la ciudad de inicio (ciudad 0) del final para no afectar la evaluación final
    s.remove(0)
    return s

# Guided Local Search (GLS) con primera mejora
def guided_local_search_first_improvement(weight: np.ndarray, eta: float, max_iter=10000, max_unimproved=100):
    n_cities = weight.shape[0]
    ...
    new_s = local_search_first_improvement(weight, init_s=current_s, penalty=penalty, eta=eta, max_unimproved=max_unimproved)
...
def run_gls(paths, eta, max_iter, max_unimproved):
    for path in paths:
        ...
        for i in range(30):
            opt = guided_local_search_first_improvement(weight, eta, max_iter, max_unimproved)
